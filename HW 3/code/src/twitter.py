...
# Note that I have scikit-learn 0.20.1 version, so the line below no longer works
...
def select_param_linear(X, y, kf, metric="accuracy"):
    ...
    arrayC = np.array([cv_performance(SVC(C=c, kernel = 'linear'), X, y, kf, metric) for c in C_range])
    return arrayC
    ### ========== TODO : END ========== ###
...
def performance_test(clf, X, y, metric="accuracy"):
    ...
    if metric == "AUROC":
        bestC = 1
    print ('Linear SVM test based on ' + str(metric) + ' with C = ' + str(bestC) + ':')
...
def main() :
    ...
    testSetX = X[560:,:]
    testSety = y[560:]
    #print ('Dimension for training set X: ' + str(trainingSetX.shape))
    #print ('Dimension for training set y: ' + str(trainingSety.shape))
    #print ('Dimension for test set X: ' + str(testSetX.shape))
    #print ('Dimension for test set y: ' + str(testSety.shape))
    ...
    for metric in ['accuracy', 'F1-Score', 'AUROC']:
        best = select_param_linear(trainingSetX, trainingSety, skf, metric)
        print (best)
    # part 3: train linear-kernel SVMs with selected hyperparameters
    clf100 = SVC(C = 100, kernel = 'linear')
    clf100.fit(trainingSetX, trainingSety)
    clf1 = SVC(C = 1, kernel = 'linear')
    clf1.fit(trainingSetX, trainingSety)
    # part 3: report performance on test data
    for metric in ['accuracy', 'F1-Score']:
        print (performance_test(clf100, testSetX, testSety, metric))
    for metric in ['AUROC']:
        print (performance_test(clf1, testSetX, testSety, metric))
    ### ========== TODO : END ========== ###
